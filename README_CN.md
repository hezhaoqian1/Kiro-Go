...
Claude 请求在构建上游思考提示时会遵循显式 `thinking.budget_tokens`。未指定时默认 8192；提供 `max_tokens` 时，默认预算不超过它的一半，为回答留出空间。Token 计数使用相同提示。这些参数是给上游的提示，不保证响应时间，也不保证上游返回可见的思考内容。

## Claude 流式兼容边界

当 `/v1/messages` 的上游正常 EOF 但缺少 `stopReason` 时，只有尚未发送回答或思考 SSE 才会执行有界重试。重试耗尽或已经开始输出后，只要响应包含非空回答文本且不处于未结束的标签思考块中，就补充 `end_turn`。已经开始的响应不会重试，以免重复输出。这兼容了部分 Kiro profile 的回答（包括账号连通性测试）。

该回退是兼容性判断，并不能证明回答在语义上完整。缺少终止信号的纯思考或空白响应、传输错误和损坏事件帧仍然报错，即使已经开始输出；短回答测试通过不代表所有长回答均可用。本次 EOF 兼容变更不改变 OpenAI 协议或非流式的 EOF 处理行为。
